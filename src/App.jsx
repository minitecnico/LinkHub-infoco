...
const ToolRemoveBg = () => {
  ...
  return (
    <div className="p-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2.5rem] shadow-xl">
      ...
      {!resultImage ? (
        <label className="block border-2 border-dashed border-white/10 rounded-2xl p-6 cursor-pointer text-center hover:bg-white/5 transition-all">
        ...
        <div className="text-center animate-in zoom-in duration-300">
          <img src={resultImage} className="max-h-40 mx-auto mb-4 rounded-xl shadow-lg border border-white/10" alt="resultado" />
          <div className="flex gap-2">
            <a href={resultImage} download="sem-fundo.png" className="flex-1 bg-blue-600 py-3 rounded-2xl text-xs font-black text-white uppercase text-center">Baixar</a>
            <button onClick={() => setResultImage(null)} className="p-3 bg-white/10 rounded-2xl text-white hover:bg-white/20"><RefreshCw size={18} /></button>
          </div>
        </div>
      )}
    </div>
...
export default function LinkHub() {
  ...
  const [data, setData] = useState(() => {
    const defaultData = {
      profile: { 
        name: 'Infoco Gestão Pública®', 
        bio: 'Inovação e Transparência na Gestão Municipal' 
      },
      links: [
      ...
      appearance: { 
        primaryColor: '#0052D4', 
        background: THEMES.blue.bg 
      },
      social: { 
        instagram: 'https://instagram.com/infocogestaopublica', 
        whatsapp: '5573981019313' 
      }
    };
  ...
  if (view === 'public') {
    return (
      <div className="min-h-screen text-white flex flex-col items-center py-16 px-6 transition-all duration-700" style={{ background: data.appearance.background }}>
        ...
        <main className="w-full max-w-md space-y-4">
          {data.links.map(link => (
            <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="flex items-center p-4 rounded-[2rem] bg-white/5 border border-white/10 backdrop-blur-xl hover:scale-[1.02] transition-all shadow-lg group">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/10 group-hover:bg-white/20">
        ...
        <footer className="mt-12 flex items-center gap-8">
          <a href={data.social.instagram} target="_blank" rel="noreferrer" className="opacity-60 hover:opacity-100 transition-all"><Instagram size={24} /></a>
          <a href={`https://wa.me/${data.social.whatsapp}`} target="_blank" rel="noreferrer" className="opacity-60 hover:opacity-100 transition-all"><WhatsAppIcon size={24} /></a>
          <button onClick={() => setIsLoginOpen(true)} className="w-11 h-11 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 opacity-60 hover:opacity-100 hover:bg-white hover:text-black transition-all"><Settings size={20} /></button>
        </footer>

        {isLoginOpen && (
          <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
            <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm text-slate-900 shadow-2xl">
              <h2 className="text-2xl font-black mb-6 tracking-tighter">Acesso Admin</h2>
              <input 
              ...
              />
              <button onClick={handleLogin} className="w-full p-4 bg-blue-600 text-white rounded-2xl font-black shadow-lg hover:bg-blue-700 transition-all">Entrar</button>
              <button onClick={()=>setIsLoginOpen(false)} className="w-full mt-4 text-slate-400 font-bold text-sm">Voltar ao Site</button>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row text-slate-900">
      <aside className="w-80 bg-white border-r p-8 hidden md:flex flex-col shadow-sm">
        ...
        <div className="p-4 bg-blue-50 text-blue-700 rounded-2xl font-bold flex items-center gap-3 border border-blue-100"><Layers size={20}/> Gerenciar Links</div>
        <button onClick={() => setView('public')} className="mt-auto p-4 bg-red-50 text-red-600 rounded-2xl font-bold flex items-center gap-2 hover:bg-red-100 transition-colors"><LogOut size={20}/> Sair</button>
      </aside>

      <main className="flex-1 p-8 md:p-16 overflow-y-auto">
        <div className="max-w-3xl mx-auto space-y-10">
          <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
            <h3 className="text-xl font-black mb-6 flex items-center gap-2"><Palette size={20} className="text-blue-600" /> Temas</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {Object.keys(THEMES).map(k => (
                <button key={k} onClick={() => setData({...data, appearance: { primaryColor: THEMES[k].primary, background: THEMES[k].bg }})} className="p-3 border-2 border-slate-100 rounded-2xl hover:border-blue-500 transition-all">
          ...
          <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
            <h3 className="text-xl font-black mb-6">Informações</h3>
            <div className="space-y-4">
               <input className="w-full p-4 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-blue-500" value={data.profile.name} onChange={(e)=>setData({...data, profile: {...data.profile, name: e.target.value}})} placeholder="Nome" />
               <input className="w-full p-4 bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-blue-500" value={data.profile.bio} onChange={(e)=>setData({...data, profile: {...data.profile, bio: e.target.value}})} placeholder="Bio" />
            </div>
          </section>
          ...
          <section className="space-y-4">
             <div className="flex justify-between items-center px-2">
                ...
                <button onClick={() => setData({...data, links: [{id: Date.now(), title: 'Novo Link', url: 'https://', icon: 'link', active: true}, ...data.links]})} className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-lg">+ Novo</button>
             </div>
             {data.links.map(l => (
